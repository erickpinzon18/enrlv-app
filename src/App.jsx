...
function RequireAuth({ children }) {
  const isLoggedIn = sessionStorage.getItem('enrlv_auth') === 'true';
  if (!isLoggedIn) {
...
function DashboardLayout() {
  ...
  // Build context to pass to child routes
  const routeContext = {
    individualData, setIndividualData,
    grupalData, setGrupalData,
    masivoData, setMasivoData,
    practicaData, setPracticaData,
    studentLists, handleAddStudent, handleRemoveStudent,
    showAlert, showPassword,
    handleGeneratePDF,
    currentTab,
  };

  const isConfigTab = currentTab === 'configuracion';
    ...
    <div className="bg-gray-100 h-screen flex overflow-hidden font-sans">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <main className="flex-1 overflow-auto p-4 md:p-8">
          <div className={`${isConfigTab ? 'max-w-7xl' : 'max-w-4xl'} mx-auto bg-white rounded-lg shadow-lg border border-gray-200 overflow-hidden min-h-[600px]`}>
            <div className="p-6 md:p-10">
              ...
              <Routes>
                ...
                <Route
                  path="configuracion"
                  element={<ConfigPanel showAlert={showAlert} />}
                />
              </Routes>

              {/* Generate Button - hidden on config tab */}
              {!isConfigTab && (
